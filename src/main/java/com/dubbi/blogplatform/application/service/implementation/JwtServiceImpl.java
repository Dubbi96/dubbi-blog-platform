...
public class JwtServiceImpl implements JwtService {
    ...
    // JwtService 인터페이스의 게터 메소드 구현
    @Override
    public String getSecretKey() {
        return secretKey;
    }
    /**인터페이스를 사용하며 @Getter를 사용하기 위해서 추가된 메소드*/
    @Override
    public Long getAccessTokenExpirationPeriod() {
        return accessTokenExpirationPeriod;
    }
     ...
     * 유효하지 않다면 빈 Optional 객체 반환
     */
    public Optional<String> extractNickname(String nickname) {
        try {
            // 토큰 유효성 검사하는 데에 사용할 알고리즘이 있는 JWT verifier builder 반환
            return Optional.ofNullable(JWT.require(Algorithm.HMAC512(secretKey))
                    .build() // 반환된 빌더로 JWT verifier 생성
                    .verify(nickname) // accessToken을 검증하고 유효하지 않다면 예외 발생
                    .getClaim(EMAIL_CLAIM) // claim(nickname) 가져오기
                    .asString());
        } catch (Exception e) {
            log.error("액세스 토큰이 유효하지 않습니다.");
            return Optional.empty();
